...
<div class="container mx-auto px-4 mt-8">
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-8 max-w-lg mx-auto">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6 text-center">Editar Reservación</h1>
        ...
        <form action="{{ route('reservaciones.update', $reservacion->idReservacionPK) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <select name="idUsuarioFK" id="idUsuarioFK" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600" required>
                        ...
                        @foreach($usuarios as $usuario)
                            <option value="{{ $usuario->id }}"
                                {{ old('idUsuarioFK', $reservacion->idUsuarioFK) == $usuario->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="fechaViaje" id="fechaViaje" value="{{ old('fechaViaje', $reservacion->fechaViaje->format('Y-m-d')) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600" required>
            </div>

            <div class="mb-4">
                <label for="numeroPersonas" class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Número de Personas:</label>
                <input type="number" name="numeroPersonas" id="numeroPersonas" value="{{ old('numeroPersonas', $reservacion->numeroPersonas) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600" required min="1">
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="pensionElegida" id="pensionElegida" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600">
                    ...
                    <option value="Completa" {{ old('pensionElegida', $reservacion->pensionElegida) == 'Completa' ? 'selected' : '' }}>Completa</option>
                    <option value="Media" {{ old('pensionElegida', $reservacion->pensionElegida) == 'Media' ? 'selected' : '' }}>Media Pensión</option>
                    <option value="Solo Desayuno" {{ old('pensionElegida', $reservacion->pensionElegida) == 'Solo Desayuno' ? 'selected' : '' }}>Solo Desayuno</option>
                </select>
            </div>
                ...
                <select name="idSucursalFK" id="idSucursalFK" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600">
                    ...
                    @foreach($sucursales as $sucursal)
                        <option value="{{ $sucursal->idSucursal }}"
                            {{ old('idSucursalFK', $reservacion->idSucursalFK) == $sucursal->idSucursal ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                <label for="idHotelFK" class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Hotel (Opcional):</label>
                <select name="idHotelFK" id="idHotelFK" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600">
                    ...
                    @foreach($hoteles as $hotel)
                        <option value="{{ $hotel->idHotel }}"
                            {{ old('idHotelFK', $reservacion->idHotelFK) == $hotel->idHotel ? 'selected' : '' }}>
            ...
            <div class="mb-6">
                <label for="idVueloFK" class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Vuelo (Opcional):</label>
                <select name="idVueloFK" id="idVueloFK" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600">
                    ...
                    @foreach($vuelos as $vuelo)
                        <option value="{{ $vuelo->idVueloPK }}"
                            {{ old('idVueloFK', $reservacion->idVueloFK) == $vuelo->idVueloPK ? 'selected' : '' }}>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="totalCotizacion" id="totalCotizacion" value="{{ old('totalCotizacion', $reservacion->totalCotizacion) }}" 
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Calcular la cotización inicial al cargar la página y con old() values
        calculateTotalCotizacion();
    });
</script>
@endsection